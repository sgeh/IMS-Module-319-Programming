...
namespace Bwz.Rappi
{
    public partial class FrmTicTacToe_v3 : Form
    {
        ...
        {
            return _fields[row][0] == _currentPlayer
                && _fields[row][1] == _currentPlayer
                && _fields[row][2] == _currentPlayer;
        }

        private bool ColumnHasWinPosition(int column)
        {
            return _fields[0][column] == _currentPlayer
                && _fields[1][column] == _currentPlayer
                && _fields[2][column] == _currentPlayer;
        }
